# Python & Django Notes 📘

This repository contains structured notes from my learning journey:

- **Core Python** (Basic → Advanced)
- **Django Web Development**
- **Django REST Framework (DRF)**
- **Projects** (Basic, Advanced, Capstone)

## Learning Path
1. Python Basics → Advanced Python  
2. Django → Django REST Framework  
3. Projects (5 total: 2 Basic, 3 Advanced/Capstone)

## Quick Links

### 🐍 Python Basics
- [Introduction](01-python-basics/01-introduction.md)
- [Features](01-python-basics/02-features.md)
- [Installation](01-python-basics/03-installation.md)
- [Flavours & Versions](01-python-basics/04-flavours-and-versions.md)
- [Identifies & Keywords](01-python-basics/05-identifiers-and-keywords.md)
- [Data types & Number systems](01-python-basics/06-data-types.md)
- [Complex, Boolean, String, Type Casting & Immutability in Python](01-python-basics/07-complex-bool-string-type-casting-immutability.md)
- [Complex, Boolean, String, Type Casting & Immutability in Python](01-python-basics/07-complex-bool-string-type-casting-immutability.md)
- [Immutability vs Mutability in Python](01-python-basics/08-collections-and-mutability.md)